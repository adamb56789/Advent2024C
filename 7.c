...
static i64 myParseInt64(char **s) {
    i64 value = 0;
    while (**s >= '0' && **s <= '9') {
        value = value * 10 + (**s - '0');
        (*s)++;
...
static int reverseDfs(const int *equation, const int i, const i64 acc) {
...
i64 totalCalibrationResult(const char *ptr, const char *end) {
    ...
    int equation[12];
    while (ptr < end - 1) {
        const i64 testValue = myParseInt64(&ptr);
        ptr++;
...
static i64 splitInt64(const i64 a, const i64 b) {
...
static int reverseDfsConcat(const int *equation, const int i, const i64 acc) {
    if (i == 0) return acc == equation[0];
